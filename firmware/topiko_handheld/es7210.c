...
#include "esp_log.h"
#include "esp_check.h"
#include "esp32-hal-i2c.h"   // Arduino-HAL: teilt sich den Bus mit Wire
...
static esp_err_t es7210_write_reg(es7210_dev_handle_t handle, uint8_t reg_addr, uint8_t reg_val)
{
    ...
    return i2cWrite(handle->i2c_port, handle->i2c_addr, buf, 2, 1000);
}

#if 0   /* Original-Implementierung (Legacy-I2C-Treiber, kollidiert mit Wire) */
static esp_err_t es7210_write_reg_legacy(es7210_dev_handle_t handle, uint8_t reg_addr, uint8_t reg_val)
{
    ESP_RETURN_ON_FALSE(handle, ESP_ERR_INVALID_ARG, TAG, "invalid device handle");
...
#endif  /* Ende Legacy-Implementierung */
